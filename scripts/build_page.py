...
def challenge_visual(kind, anchor, link):
    ...
    return f'<figure class="challenge-media" data-kind="{kind}">{media}<figcaption>{caption} <a class="evidence-link" href="#{anchor}">{link} ↓</a></figcaption></figure>'
...
<div class="resource-buttons"><a class="button button-dark" href="assets/documents/paper.pdf">Paper ↗</a><a class="button" href="assets/documents/appendix.pdf">Appendix ↗</a><a class="button" href="#full-video">Overview video ↗</a><span class="release-status"><i></i>Code not yet released</span></div>
</div></section>
...
'''
html += film('supplementary', 'overview video', loop=False, button_label='Play video') + '<figcaption>3 min · Playback speeds are marked in the video. <a href="assets/media/supplementary.mp4" download>Download video ↓</a></figcaption></figure>'
...
html += '</div></section><section class="section" id="challenges"><div class="container">'
html += heading('01', 'From challenges to design', 'Challenges &amp; Insights', 'HiFun structures what the robot explores, how actions are evaluated, and how human guidance enters learning.')
challenges = [
    ('High-dimensional exploration',
     ...
     'skill',
     'coordination-evidence', 'See the coordination comparison'),
    ('Ambiguous credit assignment',
     ...
     'critic',
     'value-evidence', 'Inspect the multi-head critic evidence'),
    ...
    ('Heterogeneous human supervision',
     ...
     'iaw',
     'iaw-evidence', 'See the intervention-weighting ablation')]
for i, (topic, challenge, insight, response, media_kind, anchor, link) in enumerate(challenges, 1):
    html += f'''<article class="challenge-row"><div class="challenge-problem"><p class="challenge-label">0{i} / Challenge</p><h3>{topic}</h3><p>{challenge}</p></div><div class="challenge-response"><p class="challenge-label text-coord">HiFun insight</p><h4>{insight}</h4><p>{response}</p></div>{challenge_visual(media_kind, anchor, link)}</article>'''
...
<div class="headline-metrics" aria-label="Main evaluation summary"><div><strong>295<span>/300</span></strong><p>successful evaluation trials</p></div><div><strong>6</strong><p>functional manipulation tasks</p></div><div><strong>2</strong><p>dexterous hand embodiments</p></div><div><strong>≤60<span> min</span></strong><p>online HIL per task<a href="#training-cost" aria-label="See full training costs">*</a></p></div></div>
<p class="source-note hero-note">98.3% mean success. 50 trials per task within the trained workspace. Online HIL is one training stage; the full pipeline averages approximately 109 min/task. <a href="assets/documents/paper.pdf#page=6">Evaluation details ↗</a></p></div></section>
<section class="section section-white" id="demos"><div class="container">'''
html += heading('02', 'What makes these tasks hard', 'Precise contact · Arm–hand coordination · Reactive recovery', 'Precise tool use, contact-rich rotation, and constrained-space manipulation require precise fingertip placement and force regulation, synchronized arm–hand motion, and recovery after small errors. Here, a narrow opening blocks direct grasping. The robot must lever the exposed handle into reach, form a precision grasp, and extract it without slipping, trapping the tool, or colliding with the opening.')
html += '<div class="demo-layout"><figure>' + film('thin-handle', 'Watch the contact sequence') + '<figcaption>Thin-Handle Retrieval · Sharpa hand · source timing preserved</figcaption></figure><ol class="contact-sequence">'
for i, (title, text) in enumerate([('Establish contact', 'Place the fingers on the exposed, task-relevant part of the handle.'), ('Create clearance', 'Synchronize arm motion and multi-finger levering to bring the handle within reach.'), ('Secure the tool', 'Transition to a precision grasp without losing contact.'), ('Complete the retrieval', 'Maintain coordinated arm motion and finger contact to avoid trapping or collision at the opening.')], 1):
    html += f'<li><span>0{i}</span><div><h3>{title}</h3><p>{text}</p></div></li>'
html += '</ol></div><p class="takeaway"><span>Task challenge</span>Small contact errors can break an entire manipulation sequence. Recovery must be learned alongside precise contact and coordination.</p></div></section><section class="section" id="insights"><div class="container">'
html += heading('03', 'Experimental evidence', 'Evidence behind the design', 'Three views of the evidence: adapting finger contact, learning arm–skill coordination, and using corrections selectively.')
html += '''<article class="insight" id="contact-evidence"><div class="insight-heading"><span class="insight-number text-skill">01</span><div><p class="eyebrow text-skill">Contact-aware hand skills</p><h3>The fingers can recover a lost contact.</h3><p>A collision rotates the key and breaks thumb contact. The residual hand skill adjusts the fingers to restore contact without arm re-alignment.</p></div></div><figure class="wide-evidence">'''
html += film('contact-recovery', 'Watch contact recovery · 1×', speed='1×')
html += '''<figcaption><strong>Contact lost → finger adjustment → contact restored.</strong> Submitted supplementary video, 0:27–0:34. Key turning is an additional skill example, outside the six-task main evaluation.</figcaption></figure><figure class="recovery-sequence"><div class="recovery-scroll" role="region" tabindex="0" aria-label="Five-frame contact recovery sequence; scroll horizontally for detail"><button class="zoom-figure" data-zoom="assets/figures/key-recovery-sequence.png" aria-label="Enlarge the five-frame contact recovery sequence"><img src="assets/figures/key-recovery-sequence.png" width="7421" height="1109" loading="lazy" alt="Five frames: an unexpected hand–object collision reverses the key rotation; abnormal thumb contact triggers reference-state rewind; residual finger adjustment restores thumb contact. Original arrows, equations and timestamps are preserved."></button></div><figcaption>From collision to restored thumb contact: the reference rewinds and the residual skill adjusts finger execution. <span class="scroll-hint">Swipe or use the arrow keys to inspect all five frames.</span> Click the figure to enlarge.</figcaption></figure><div class="evidence-strip"><div><strong>19<span>/20</span></strong><h4>Unseen tool orientations</h4><p>Fixed-arm Angle-Spreader skill. Toolbox yaw 0–30°; tool pitch/roll 0–20°.</p></div><div><strong>18<span>/20</span></strong><h4>Unseen drill orientations</h4><p>Fixed-arm Bit Removal skill, with drill yaw up to 30°.</p></div><div><strong>18<span>/20</span></strong><h4>External contact disturbances</h4><p>Fixed-arm Angle-Spreader skill under strong wrench perturbations.</p></div></div><p class="source-note">These experiments support local recovery around a demonstrated contact structure. <a href="assets/documents/additional-evidence.pdf">Additional evaluation, rebuttal ↗</a></p></article>
<article class="insight" id="coordination-evidence"><div class="insight-heading"><span class="insight-number text-coord">02</span><div><p class="eyebrow text-coord">Skill-conditioned coordination</p><h3>A hand skill still needs the right timing.</h3><p>Giving a baseline access to the same learned skill improves performance. Learning where to align the arm and when to activate that skill closes a further gap.</p></div></div><div class="comparison-layout"><figure class="chart-panel"><figcaption class="chart-title">Mean task success<span>Four Sharpa tasks · matched training budget</span></figcaption><div class="bar-chart" role="img" aria-label="Four-task mean reported success: flat HIL-SERL with the same skill intervention interface 12.5 percent, Skill-HIL 46 percent, HiFun 100 percent.">'''
for label, sub, value, color in [('Flat HIL-SERL', 'same skill intervention interface', '12.5', 'neutral'), ('Skill-HIL', 'learned skill as an action primitive', '46', 'blue'), ('HiFun', 'skill-conditioned coordination', '100', 'green')]:
    html += f'<div class="bar-row"><div class="bar-label">{label}<small>{sub}</small></div><div class="bar-track"><span style="--value:{value}%" class="bar {color}"></span></div><strong>{value}%</strong></div>'
html += '''<div class="bar-axis" aria-hidden="true"><span>0</span><span>50</span><span>100%</span></div></div><p class="source-note">Separate from the six-task 98.3% headline result. <a href="assets/documents/additional-evidence.pdf">Rebuttal comparison ↗</a></p></figure><aside class="evidence-aside"><p class="eyebrow">Additional-data control</p><h4>More data alone<br>leaves a gap.</h4><div class="paired-numbers"><div><strong>35<span>/50</span></strong><p>Skill-HIL<br>with 2× training data</p></div><div><strong class="text-coord">50<span>/50</span></strong><p>HiFun<br>standard budget</p></div></div><p class="small">Angle-Spreader Retrieval. Reported failures involve contact regions, activation timing and subsequent collisions.</p></aside></div></article>
<article class="insight" id="value-evidence"><div class="insight-heading"><span class="insight-number">03</span><div><p class="eyebrow">Value-guided learning</p><h3>Learn activation timing.<br>Weight useful corrections.</h3><p>Separate value heads train spatial alignment and skill activation. Intervention advantage weighting (IAW) gives more imitation weight to corrections with higher estimated value than the current policy action.</p></div></div>
<div class="analysis-grid"><div class="analysis-block"><p class="chart-kicker">Multi-head critic</p><h4>Activation closer to the target</h4><div class="error-comparison"><div><strong>4.58<small> cm</small></strong><span>without multi-head</span></div><span aria-hidden="true">→</span><div><strong class="text-coord">2.00<small> cm</small></strong><span>HiFun</span></div></div><p>95th-percentile TCP-to-target error at activation, measured on Angle-Spreader Retrieval.</p></div><div class="analysis-block" id="iaw-evidence"><p class="chart-kicker">Intervention advantage weighting</p><h4>Selective imitation improves success</h4><div class="ablation-values"><div><strong class="text-coord">100%</strong><span>HiFun</span></div><div><strong>60%</strong><span>Uniform<br>intervention BC</span></div><div><strong>40%</strong><span>No intervention<br>BC loss</span></div></div><p>The same success rates are reported on each of Angle-Spreader Retrieval and Bit Removal.</p></div></div>'''
html += detail('Inspect the component evidence', figure('coordination-analysis', 'Coordination component analysis', 'Original Appendix Fig. 4. IAW estimates usefulness through a critic; it does not directly observe the true quality of a human correction.') + '<p class="source-note"><a href="assets/documents/paper.pdf#page=7">Main paper, Table 2 ↗</a> · <a href="assets/documents/appendix.pdf#page=6">Appendix D.3 ↗</a></p>')
html += '</article></div></section><section class="section section-white" id="method"><div class="container">'
html += heading('04', 'Method', 'Two levels.<br>One physical interaction.', 'A learned hand skill executes precise contact. A coordination policy controls arm motion and hand-skill activation through a compact interface.')
html += '''<div class="method-diagram" role="group" aria-label="HiFun execution architecture"><div class="method-observation"><span class="diagram-label">Observation</span><p>RGB images · arm–hand state · contact feedback</p></div><div class="policy-pair"><div class="policy policy-skill"><span class="policy-id">π<sub>skill</sub></span><h3>How to contact</h3><p>Kinesthetic reference + residual RL</p><div class="policy-output">Contact-aware finger commands</div><small>Weights frozen during coordination learning;<br>execution remains feedback-conditioned.</small></div><div class="policy policy-coord"><span class="policy-id">π<sub>coord</sub></span><h3>Where / when to activate</h3><p>Learned arm–hand coordination</p><div class="policy-output">6D arm motion Δx + activation α</div><small>Arm motion and skill activation are learned;<br>a fixed move–stop–act sequence is not imposed.</small></div></div><div class="execution-output"><span aria-hidden="true">↓</span><div><strong>Coordinated arm–hand execution</strong><p>The activation value blends the hand-skill command with the neutral hand configuration.</p></div></div></div>'''
html += detail('Training stages and complete framework', '<ol class="training-stages"><li><strong>Acquire a contact-aware hand skill.</strong> Start with a kinesthetic reference and refine finger execution with residual RL on the real robot.</li><li><strong>Warm-start coordination.</strong> Generate 100 physical DMP rollouts from an arm-and-activation demonstration, then pretrain the coordination policy.</li><li><strong>Refine with online interaction.</strong> Use skill-level human corrections, separate value heads and intervention advantage weighting.</li></ol>' + figure('method', 'Complete HiFun framework', 'Main paper, Fig. 2. The hand skill is also the human operator’s intervention interface. <a href="assets/documents/paper.pdf#page=3">Read the method ↗</a>'))
html += '</div></section><section class="section" id="results"><div class="container">'
html += heading('05', 'Real-world evaluation', 'Six tasks.<br>Functional outcomes, measured.', 'Each task is evaluated over 50 trials with randomized object poses inside the trained workspace. Success requires completing the functional outcome.')
...
html += '</div>'
html += '<div class="subsection-heading" id="recovery-transfer"><p class="eyebrow">Beyond nominal rollouts</p><h3>Recovery and local transfer</h3><p>HiFun adjusts its approach after a tray moves and reuses a learned hand skill across different tool sizes.</p></div><div class="extension-grid">'
html += '<figure>' + film('spreader-disturbance', 'Tray disturbance') + '<figcaption><strong>Changed object placement.</strong> A moved tray requires coordination to adapt the approach.</figcaption></figure>'
html += '<figure>' + film('size-transfer', 'Same skill, different sizes', speed='2×') + '<figcaption><strong>7.5 cm → 6.5 cm / 5.5 cm.</strong> The same hand skill transfers to smaller tool variants.</figcaption></figure></div>'
...
html += '<section class="section section-white" id="training-cost"><div class="container">' + heading('06', 'Learning cost & autonomy', 'What does the training budget include?', 'Online HIL fine-tuning averages 46.2 minutes per task. Including hand-skill learning and preparation, the reported full pipeline averages approximately 109 minutes per task.')
html += '<div class="cost-layout"><div class="cost-summary"><strong>~109<span> min/task</span></strong><p>Reported full-pipeline average</p><a href="assets/documents/additional-evidence.pdf">Training-cost clarification ↗</a></div><div class="cost-stages">'
for name, time in [('Kinesthetic reference', '&lt;5 min'), ('Hand-skill residual RL', '~30 min'), ('Reward-classifier data', '8.8 min avg.'), ('Automated DMP rollouts', '18.8 min avg.'), ('Online HIL coordination', '46.2 min avg.')]:
    html += f'<div><span>{name}</span><strong>{time}</strong></div>'
html += '</div></div><div class="subsection-heading"><h3>Human guidance decreases during training.</h3><p>Intervention rate complements success: it measures how much human correction is still needed. The original curves below show intervention rate and cycle time across the six tasks.</p></div><div class="training-figure">'
html += figure('training-curves', 'Intervention rate and cycle time across six tasks', 'Original Appendix Fig. 3. Running averages over 20 episodes. The original figure uses orange for HiFun and blue for HIL-SERL. <a href="assets/documents/appendix.pdf#page=5">Source ↗</a>')
html += '</div></div></section><section class="section" id="resources"><div class="container">'
html += heading('07', 'Explore the work', 'From the behavior<br>to the details.', 'Read the method and evaluation, watch the overview video, or explore the additional evidence introduced during review.')
html += '<div class="resource-list">'
for name, filename, desc in [('Paper', 'paper.pdf', 'Method, main results and limitations'), ('Supplemental appendix', 'appendix.pdf', 'Protocols, learning curves and component analysis'), ('Additional evidence', 'additional-evidence.pdf', 'Rebuttal: Skill-HIL, recovery and training-cost clarification')]:
    html += f'<a href="assets/documents/{filename}"><span><strong>{name}</strong><small>{desc}</small></span><span aria-hidden="true">↗</span></a>'
html += '<div class="resource-unavailable"><span><strong>Code</strong><small>Implementation repository not yet publicly released</small></span><span class="availability">Not yet released</span></div></div>'
html += detail('Scope and next directions', '<p>HiFun relies on task-relevant hand skills learned around kinesthetic priors. New contact structures may require additional priors or adaptation. The current evaluation establishes functional execution within trained workspaces and local contact variations, rather than language-conditioned or open-ended task generalization.</p><p>Reusable hand-skill libraries and integration with high-level task-selection models are research directions. They are not evaluated capabilities of the current system.</p><p class="source-note"><a href="assets/documents/paper.pdf#page=8">Main paper, Limitations ↗</a></p>')
html += '''<div class="citation-section"><div class="citation-heading"><h3>Citation</h3><button class="copy-button" id="copy-citation">Copy BibTeX</button></div><pre id="citation"><code>@inproceedings{huang2026hifun,
...
<footer><div class="container footer-inner"><a class="brand" href="#top">HiFun</a><p>Contact-aware skills. Learned coordination.</p><a href="#top">Back to top ↑</a></div><div class="container footer-credit">Visual direction inspired by <a href="https://siriyep.github.io/last-hd-project-page/">LaST-HD</a>. Robot footage and research materials belong to the HiFun project.</div></footer>
